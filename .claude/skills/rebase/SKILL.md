...
- **Metrics endpoint on failure**: upstream's `collect()` yields nothing until the first refresh lands and keeps serving the previous values after a failed one, always with a 200. This project returns 503 until a refresh has succeeded, so a scrape never records values from a refresh that could not read the repository. Upstream's equivalent knob is `EXIT_ON_ERROR`, which exits the process instead.
